...
def main():
    ...
    import os
    import re
    DEFAULT_ARCHIVE_FILE = "/Users/hadden/Papers/11_breaking_chains/03_data/migration_hd110067/run_mtot_0.03_mtp_3_K_100_continued.sa"
    parser = argparse.ArgumentParser()
    parser.add_argument("--reload",action="store_true")
    parser.add_argument("--archive",help="Input archive file name.",default=DEFAULT_ARCHIVE_FILE)
    parser.add_argument("--save",required=False,help="File to save plot.")
    args = parser.parse_args()
    save_file_name = re.sub(".sa","_plot_data.npz",args.archive)
    f = re.search(r"mtot_(\d+\.\d+)",args.archive).groups()[0]
    mtp = re.search(r"mtp_(\d+)",args.archive).groups()[0]
    K = re.search(r"K_(\d+)",args.archive).groups()[0]
    if args.reload or not os.path.exists(save_file_name):
        sa = rb.Simulationarchive(args.archive)
        data = process_sa(sa)
        np.savez_compressed(save_file_name,**data)
    else:
        data = np.load(save_file_name)
    print(list(data.keys()))
    title = r"$m_{{s}}={}M_\mathrm{{Pluto}}~;~f={}\%~;~ K={}$".format(mtp,int(100*float(f)),K)
    fig,ax = plot_resonances(data,title = title)
    if args.save:
